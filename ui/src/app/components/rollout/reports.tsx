              ...
              const mResults = b.status?.metricResults;
                mResults.forEach((element:any,index:number) => {
                 // if(element.name.includes('opsmx')){
                    ...
                    if (element.measurements[0]?.metadata['job-name']) {
                      fetchEndpointURL(props.reportsInput.appName, props.reportsInput.resourceName, props.reportsInput.nameSpace, props.reportsInput.version, element.measurements[0]?.metadata['job-name'],index);
                    }
                 // }
                });
